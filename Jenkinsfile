pipeline {
    ...
    environment {
        IMAGE_NAME = "amoghazy/eos-gateway-api"
      
    }
    stages {
        stage('Checkout SCM') {
            steps {
                git credentialsId: '', url: 'https://github.com/amoghazy-organization/5-eos-micro-services-gateway-api-build.git', branch: 'main'
            }
        }
                    ...
                    dir('charts') {
                        withCredentials([usernamePassword(credentialsId: 'jfrog-cred', usernameVariable: 'username', passwordVariable: 'password')]) {
                          sh ''' 
                            ...
                            /usr/local/bin/helm package gateway-api
                            curl -u ecomadmin:$password -T /home/jenkins/agent/workspace/$JOB_NAME/charts/gateway-api-1.0.tgz "https://triallekevd.jfrog.io/artifactory/ecom-helm-local/gateway-api-1.0.tgz"
